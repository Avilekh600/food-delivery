...
function Card({name,image,id,price,type}) {
  ...
  return (
    <div className='w-60 h-90 p-4 bg-white rounded-md flex flex-col gap-2 m-4 hover:border-2 border-green-400 transition-all shadow-xl'>
          ...
          <div>
                <button className='bg-green-200 w-full mt-3 p-2 rounded-md cursor-pointer hover:bg-green-300 transition-all' onClick={()=>{dispatch(AddItem({id:id, name:name, price:price,image:image,qty:1}));
                toast("Item added")
                }}>Add to Dish</button>
            </div>
    </div>
